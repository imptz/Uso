...
// M13112012
int Logic::getActiveInitialSignal(LOGIC_FUNCTION function, LOGIC_FUNCTION function1)
{
	...
	for (unsigned int i = 0; i < Config::getSingleton().getConfigData()->getConfigDataStructInitSignalsCount(); i++)
	{
		...
		{
			if (((pData[i]->function == function) || (pData[i]->function == function1)) && (!pData[i]->ignorable))
			{
