		...
		case DETECTION_PHASE_MOVE_PR:
			...
			if (result)
			{
				...
				if (_count == 0)
				{
					...
					sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 1));
				}
				else
				{
					if (pDevices[0] == nullptr)
					{
						detection = DETECTION_FAULT;
						detectionPhase = DETECTION_PHASE_STOP;
						sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 2));
					}
					else
					{
		...
		case DETECTION_PHASE_GET_FIRE:
			if (pDevices[0]->isReady())
			{
				...
				if (fire == nullptr)
				{
					detection = DETECTION_FAULT;
					detectionPhase = DETECTION_PHASE_STOP;
					sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 3));
				}
				else
				{
					if (!isFireExists(fire))
					{
						detection = DETECTION_FAULT;
						detectionPhase = DETECTION_PHASE_STOP;
						sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 4));
					}
					else
					{
		...
		case DETECTION_PHASE_SCANNER_WAIT:
			if (pDevices[0]->isReady())
			{
				...
				if (fire == nullptr)
				{
					detection = DETECTION_FAULT;
					detectionPhase = DETECTION_PHASE_STOP;
					sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 5));
				}
				else
				{
					if (!isFireExists(fire))
					{
						detection = DETECTION_FAULT;
						detectionPhase = DETECTION_PHASE_STOP;
						sendMessage(Message(MESSAGE_FROM_OFFSET_DETECTION_MANAGER, DETECTION_MANAGER_MESSAGE_CHANNELS_UPDATED, DETECTION_FAULT, 6));
					}
					else
					{
						detection = DETECTION_READY;
