...
// M061112E
			...
			(tv == rh.tv) &&
			(pc == rh.pc) &&
			(topField == rh.topField) &&
			(bottomField == rh.bottomField) &&
			(leftField == rh.leftField) &&
			(rightField == rh.rightField) &&
			(timeReturnFromeRemoteMode == rh.timeReturnFromeRemoteMode) &&
			(stopSearchToRemote == rh.stopSearchToRemote) &&
			(requestUserBeforeSearch == rh.requestUserBeforeSearch) &&
			(autoPrToZero == rh.autoPrToZero) &&
			(timeRepeatSearch == rh.timeRepeatSearch) &&
			(delayAfterReset == rh.delayAfterReset));
	}
...
// M061112E
		...
		*buffer += sizeof(bool);
		...
		topField = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);

		bottomField = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);

		leftField = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);

		rightField = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);

		timeReturnFromeRemoteMode = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);
		...
		*buffer += sizeof(bool);
		...
		timeRepeatSearch = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(int);

		delayAfterReset = *reinterpret_cast<bool*>(*buffer);
		*buffer += sizeof(bool);
	}
...
// M061112E
		...
		*reinterpret_cast<int*>(*buffer) = timeReturnFromeRemoteMode;
		*buffer += sizeof(timeReturnFromeRemoteMode);
		...
		*reinterpret_cast<bool*>(*buffer) = delayAfterReset;
...
struct ConfigDataStructProgram
{
	...
	ConfigDataStructProgram()
		:	initSignalNumber(0), 
			...
			point1(Point2<unsigned int>()),
			point2(Point2<unsigned int>()),
			nPointProgram(0),
			nasadok(0)
	{
	}
	...
	ConfigDataStructProgram(unsigned char** buffer)
	{
		...
		*buffer += sizeof(function);
		...
		point1 = *reinterpret_cast<Point2<unsigned int>*>(*buffer);
		*buffer += sizeof(point1);

		point2 = *reinterpret_cast<Point2<unsigned int>*>(*buffer);
	...
	void serialize(unsigned char** buffer)
	{
		...
		*reinterpret_cast<Point2<unsigned int>*>(*buffer) = point1;
		*buffer += sizeof(point1);

		*reinterpret_cast<Point2<unsigned int>*>(*buffer) = point2;
		*buffer += sizeof(point2);
...
struct ConfigDataStruct
{
	...
	static const unsigned int MAX_NUMBER_FV300 = 500;
	unsigned int number_Fv300;
	ConfigDataStructFv300** _Fv300;

	static const unsigned int MAX_NUMBER_Trajectory = 2000;
	unsigned int number_Trajectory;
	ConfigDataStructTrajectory** _Trajectory;

	static const unsigned int MAX_NUMBER_PRESSURE = 4000;
