			...
			case SerialDebug::SERIAL_DEBUG_MESSAGE_RECV_COMMAND:
				switch (message.par1)
				{
					case SerialDebug::COMMAND_GET_SETTINGS:
						...
						if(pConfigData->getConfigDataStructConst()->tv)
							DEBUG_PUT("   адаптер тв:                                                      есть\n\n")
						else
							DEBUG_PUT("   адаптер тв:                                                      нет \n\n")

						if(pConfigData->getConfigDataStructConst()->pc)
							DEBUG_PUT("   адаптер пк:                                                      есть\n\n")
						else
							DEBUG_PUT("   адаптер пк:                                                      нет \n\n")
													
						DEBUG_PUT("   верхнее поле при тушении:                                     %u град\n\n", pConfigData->getConfigDataStructConst()->topField)
						DEBUG_PUT("   нижнее поле при тушении:                                      %u град\n\n", pConfigData->getConfigDataStructConst()->bottomField)
						DEBUG_PUT("   левое поле при тушении:                                       %u град\n\n", pConfigData->getConfigDataStructConst()->leftField)
						DEBUG_PUT("   правое поле при тушении:                                      %u град\n\n", pConfigData->getConfigDataStructConst()->rightField)
						DEBUG_PUT("   время возврата из дистанционного режима                       %u мин\n\n", pConfigData->getConfigDataStructConst()->timeReturnFromeRemoteMode)

						if(pConfigData->getConfigDataStructConst()->stopSearchToRemote)
							DEBUG_PUT("   окончание поиска при переходе в дистанционный режим                да\n\n")
						else
							DEBUG_PUT("   окончание поиска при переходе в дистанционный режим                нет\n\n")

						if(pConfigData->getConfigDataStructConst()->requestUserBeforeSearch)
							DEBUG_PUT("   запрос оператора перед поиском очага                               да\n\n")
						else
							DEBUG_PUT("   запрос оператора перед поиском очага                               нет\n\n")

						if(pConfigData->getConfigDataStructConst()->autoPrToZero)
							DEBUG_PUT("   автоматическая установка ПР в начало координат                     да\n\n")
						else
							DEBUG_PUT("   автоматическая установка ПР в начало координат                     да\n\n")

						DEBUG_PUT("   интервал времени для повторного поиска очага                  %u мин\n\n", pConfigData->getConfigDataStructConst()->timeRepeatSearch)

						if(pConfigData->getConfigDataStructConst()->delayAfterReset)
							DEBUG_PUT("   задержка на опрос пожарной сигнализации после сброса               да\n\n")
						else
							DEBUG_PUT("   задержка на опрос пожарной сигнализации после сброса               да\n\n")
