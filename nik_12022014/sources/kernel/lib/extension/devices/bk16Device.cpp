	...
	:	IIODevice(_address, _type)
{
	...
	unsigned count = pConfigData->ioBk_count;

	inputsCount = count;
	pInputs = new Input[inputsCount];

	for (unsigned int i = 0; i < count; i++)
		if (pConfigData->ioBk[i].outputFunctionGroup != ConfigData_ioBk::OUTPUT_FUNCTION_GROUP_UNDEFINED)
			outputsCount++;
	...
	for (unsigned int i = 0; i < count; i++)
	{
		pInputs[i].projectNumber = pConfigData->ioBk[i].projectNumber;
		pInputs[i].onDeviceNumber = (pConfigData->ioBk[i].bkAddress - 1) * 8 + pConfigData->ioBk[i].numberOnDevice - 1;
		pInputs[i].state = INPUT_STATE_UNDEFINED;

		if (pConfigData->ioBk[i].outputFunctionGroup != ConfigData_ioBk::OUTPUT_FUNCTION_GROUP_UNDEFINED)
		{
			pOutputs[_ouputCount].projectNumber = pConfigData->ioBk[i].projectNumber;
			pOutputs[_ouputCount++].onDeviceNumber = (pConfigData->ioBk[i].bkAddress - 1) * 8 + pConfigData->ioBk[i].numberOnDevice - 1;
		}
	}
...
void Bk16Device::createInitFrame()
{
	...
	for (unsigned int i = 0; i < count; i++)
		if (pConfigData->ioBk[i].bkAddress < 64)
			boards[pConfigData->ioBk[i].bkAddress] = true;
	...
	for (unsigned int i = 0; i < count; i++)
		initData[DATA_OFFSET + 1 + boardsCount + (pConfigData->ioBk[i].bkAddress - 1) * 8 + pConfigData->ioBk[i].numberOnDevice - 1] = pConfigData->ioBk[i].inputFunctionGroup;
