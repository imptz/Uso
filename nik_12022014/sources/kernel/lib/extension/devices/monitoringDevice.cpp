...
void MonitoringDevice::createInitFrame()
{
	...
	unsigned int prCount = pConfigData->prPositions_count;
	unsigned int penaBakCount = pConfigData->penabak_count;
...
//	unsigned int dataLength = 10 + prCount * 4 + 1 + penaBakCount * 4; 
	...
	initData[HOUR_OFFSET] = pConfigData->constants.testingHour;
	initData[MINUTE_OFFSET] = pConfigData->constants.testingMinute;
	initData[TESTING_INFO_OFFSET] = pConfigData->constants.permissionTestingInfo;
	...
	{
		initData[FIRST_PR_INFO_OFFSET + i * 5] = pConfigData->prPositions[i].projectNumber;
		initData[FIRST_PR_INFO_OFFSET + i * 5 + 1] = pConfigData->prPositions[i].address;
		initData[FIRST_PR_INFO_OFFSET + i * 5 + 2] = pConfigData->prPositions[i].networkIndexNumber;
	...
	for (unsigned int i = 0; i < penaBakCount; ++i){
		initData[FIRST_PR_INFO_OFFSET + prCount * 5 + i * 4 + 1] = pConfigData->penabak[i].number;
		initData[FIRST_PR_INFO_OFFSET + prCount * 5 + i * 4 + 1 + 1] = pConfigData->penabak[i].level;
		initData[FIRST_PR_INFO_OFFSET + prCount * 5 + i * 4 + 2 + 1] = pConfigData->penabak[i].address;
		initData[FIRST_PR_INFO_OFFSET + prCount * 5 + i * 4 + 3 + 1] = pConfigData->penabak[i].numberOnDevice;
	}
...
void MonitoringDevice::getMessageInfo(unsigned char* pMsg, char** text, unsigned char* parameter1, unsigned char*parameter2)
{
	...
	switch (pMsg[MESSAGE_CODE_OFFSET])
	{
		...
		case MESSAGE_NUMBER_PENABAK_DATCHIK_ISPRAVEN:
			...
			*parameter2 = pMsg[MESSAGE_PAR4_OFFSET];
			break;
		case MESSAGE_NUMBER_SCANER_MEHANIZM_GUT:
			*text = const_cast<char*>(MESSAGE_TEXT_SCANER_MEHANIZM_GUT);
			*parameter1 = pMsg[MESSAGE_PAR2_OFFSET];
			*parameter2 = pMsg[MESSAGE_PAR3_OFFSET];
			break;
