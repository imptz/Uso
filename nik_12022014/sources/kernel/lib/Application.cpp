...
void Application::createLogic()
{
	...
	unsigned int count = config->programs_count;
	
	bool searching = false;
	for (unsigned int i = 0; i < count; i++)
		if ((config->programs[i].function == LOGIC_FUNCTION_SEARCHING) || (config->programs[i].function == LOGIC_FUNCTION_SEARCHING_PENA))
			searching = true;
