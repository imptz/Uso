...
bool Config::readConfigDataFromHdd(){
	Display::getSingleton().print("Config::readConfigDataFromHdd 1", 1, 12);
	if(fReadWrite)
		return false;

	Display::getSingleton().print("Config::readConfigDataFromHdd 2", 1, 12);
	...
	if (hddTaskId != HddManager::UNDEFINED_ID){
		while(HddManager::getSingleton().isTaskExecute(hddTaskId)){}
		memcpy(pConfigData, hddBuffer, sizeof(ConfigData));

		unsigned int crc = calcDataCrc();
		if(pConfigData->dataCrc == crc){
			Display::getSingleton().print("pConfigData->dataCrc == crc     ", 1, 12);
...
unsigned char Config::getPRAddressByNumber(unsigned char number){
	for (unsigned int i = 0; i < pConfigData->prPositions_count;i++){
		if (pConfigData->prPositions[i].projectNumber == number)
			return pConfigData->prPositions[i].address;
	}

	return -1;
}

unsigned char Config::getPRNumberByAddress(unsigned char address){
	for (unsigned int i = 0; i < pConfigData->prPositions_count;i++){
		if (pConfigData->prPositions[i].address == address)
			return pConfigData->prPositions[i].projectNumber;
	}

	return -1;
}

unsigned char Config::getPRIndexByAddress(unsigned char address){
	for (unsigned int i = 0; i < pConfigData->prPositions_count;i++){
		if (pConfigData->prPositions[i].address == address)
			return i;
	}
...
unsigned int Config::getConfigDataStructProgramInitSignal(unsigned int initSignalNumber, unsigned int _function, unsigned int** pList){
	unsigned int _function1 = LOGIC_FUNCTION_UNDEFINED;
	...
	for (unsigned int i = 0; i < pConfigData->programs_count; i++)
	{
		if ((pConfigData->programs[i].initSignalNumber == initSignalNumber) && ((pConfigData->programs[i].function == _function) || (pConfigData->programs[i].function == _function1)))
			count++;
	}
...
bool Config::isTvExistsInConfig(){
	return pConfigData->constants.tv;
}

bool Config::isFv300ExistsInConfig(){
	return (pConfigData->fv300_count > 0);
}
