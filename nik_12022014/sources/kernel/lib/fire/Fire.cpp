...
//	ConfigDataStructPRPosition** prp = Config::getSingleton().getConfigData()->getConfigDataStructPRPositions();
	...
	float length = fire->massCenter.distance(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(Config::getSingleton().getPRAddressByNumber(prNumber))].position) / 100.0f;
...
//		(*programs)[i].point2.y += localFires[i].prProg.y;
		...
		(*programs)[i].point1.x += (360 - static_cast<int>(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[i].channel)].orientation.x));
		(*programs)[i].point2.x += (360 - static_cast<int>(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[i].channel)].orientation.x));
		...
		if (fire->massCenter.distance(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[i].channel)].position) < 
			Config::getSingleton().getConfigData()->constants.minimumDistanceForCompactJet * 100) 
