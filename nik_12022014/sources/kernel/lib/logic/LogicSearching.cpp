...
bool LogicSearching::phaseOpenValve_Start()
{
	...
	for (unsigned int i = 0; i < actionCount; i++)
	{
		...
		unsigned int count = Config::getSingleton().getConfigData()->programs_count;
//		ConfigDataStructProgram** programs = Config::getSingleton().getConfigData()->getConfigDataStructPrograms();

		if ((count != 0) && (Config::getSingleton().getConfigData()->programs[0].function == LOGIC_FUNCTION_SEARCHING_PENA)){
			if ((actionList[i] != nullptr) && (actionList[i]->getState() != Action::STATE_ERROR))
			{
				deviceAddress = actionList[i]->getDeviceAddress();
				SAFE_DELETE(actionList[i])
...
//	ConfigDataStructPRPosition** prp = Config::getSingleton().getConfigData()->getConfigDataStructPRPositions();
	...
	if (*channelsCount > 1)
	{
		for (unsigned int i = 0; i < *channelsCount - 1; i++)
			...
			{
				if (fire->massCenter.distance(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[i].channel)].position) > 
					fire->massCenter.distance(Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[j].channel)].position))
				{
...
{
	Point3<float> pos = Config::getSingleton().getConfigData()->prPositions[Config::getSingleton().getPRIndexByAddress(localFires[i].channel)].position;
	Point3<float> posF = fire->massCenter;
}
	
	if (Config::getSingleton().getConfigData()->constants.maxPR < *channelsCount)
		*channelsCount = Config::getSingleton().getConfigData()->constants.maxPR;
...
//Log::getSingleton().add(LOG_MESSAGE_FROM_LOGIC, LOG_MESSAGE_TYPE_INFO, startLogText, 99, coolingSignal);
			...
			{
				Config::getSingleton().getConfigData()->initSignal[coolingSignal].ignorable = true;
			}
		}		
	}
}
...
//	ConfigDataStructProgram** sp = Config::getSingleton().getConfigData()->getConfigDataStructPrograms();
	...
	if (listProgramIndex != nullptr)
	{
		...
		{
			int index = indexPrInActionList(Config::getSingleton().getPRAddressByNumber(Config::getSingleton().getConfigData()->programs[listProgramIndex[i]].prNumber));
			...
			{
				if ((validActionCount + listCoolingStartCount) < Config::getSingleton().getConfigData()->constants.maxPR)
				{
					listCoolingStartIndex[listCoolingStartCount++] = listProgramIndex[i];
				}
				else
...
//	ConfigDataStructProgram** sp = Config::getSingleton().getConfigData()->getConfigDataStructPrograms();
	...
	{
		unsigned char addr = Config::getSingleton().getPRAddressByNumber(Config::getSingleton().getConfigData()->programs[listCoolingStartIndex[i]].prNumber);
		//actionStartList[i * 2] = new ActionStartProgramScanPoint(sp[listCoolingStartIndex[i]]->nPointProgram, addr);
actionStartList[i * 2] = new ActionStartProgramScanLine(Config::getSingleton().getConfigData()->programs[listCoolingStartIndex[i]].point1,
	Config::getSingleton().getConfigData()->programs[listCoolingStartIndex[i]].point2, 
	Config::getSingleton().getConfigData()->programs[listCoolingStartIndex[i]].nasadok,
