...
void LogicSearching::action()
{
	...
	switch (phase)
	{
		case PHASE_INPUT_CONTROL:
			initSignal = getActiveInitialSignal(LOGIC_FUNCTION_SEARCHING, LOGIC_FUNCTION_SEARCHING_PENA, LOGIC_FUNCTION_SEARCHING_PENA_ZATVOR);
			if (initSignal != -1)
			{
...
bool LogicSearching::phaseGateOpen_Start()
{
	...
	for (unsigned int i = 0; i < actionCount; i++)
	{
		...
		actionList[i] = new ActionGateOpen(deviceAddress, penaZatvor);
	}
...
bool LogicSearching::phaseGateClose_Start()
{
	...
	for (unsigned int i = 0; i < actionCount; i++)
	{
		if (actionList[i] != nullptr)
		{
			...
			SAFE_DELETE(actionList[i])
			actionList[i] = new ActionGateClose(deviceAddress, penaZatvor);
		}
	}
	return true;
}
